...
# Is the alpine:seqan3 image already in the docker daemon?
IID_SEQAN3="$(docker images -q alpine:seqan3 2> /dev/null)"

# Build the alpine:seqan3 docker image
docker build --iidfile iid.tmp -f alpine/cmake.dkr -t alpine:seqan3 ..

IID_SEQAN3_POST="$(cat iid.tmp)"
rm iid.tmp

# If the alpine:seqan3 image was pre-built
...
docker build --iidfile iid.tmp -f alpine/install.dkr -t hopa ..
